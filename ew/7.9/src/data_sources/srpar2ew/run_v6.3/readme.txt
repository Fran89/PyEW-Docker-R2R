...
The run_v6.3 directory contains versions of SrPar2Ew compiled
with the Earthworm 6.3 headers for Windows (.exe) and Linux (no
...
The following steps describe how to run EW6.3 with PARxCH + PARGPS.
Linux users should replace references to c:\earthworm with
/usr/local/earthworm:

1. Install the official version of Earthworm v6.3
   A. From folkworm.ceri.memphis.edu/ew-dist/v6.3index.html
      download the "Windows Binary (zip)" v6.3-win-bin.zip
      or the "Linux Binary (tgz)" v6.3-linux-bin.tar.gz.
      Linux users may prefer getting the source from
      "Source Code (tgz)" v6.3-src.tar.gz and recompiling it for
      themselves if their Linux version does not exactly match the
      provided binary.
   B. Create the directory c:\earthworm and unzip v6.3 into this
      directory.
...
3. Update srpar2ew in the earthworm directory from step 1 to the latest
   as follows:
   A. Go to the c:\earthworm\v6.3\src\data_sources\srpar2ew directory
   B. Delete the contents of this directory
   C. Replace them with the files in \sr\parxch\extras\earthworm
   D. Copy run_v6.3\srpar2ew.exe to c:\earthworm\v6.3\bin\.

4. Set up your run_symres directory
   A. Go to c:\earthworm\v6.3 and create the run_symres directory
   B. Go to c:\earthworm\v6.3\run_symres and create directories data, log,
      and params
   C. Fill the params directory with the files in run_v6.3\params
   D. Review the srpar2ew.d file and edit the AtodModelName,
      PortMode and GpsSerialPort to be appropriate for your equipment.

5. Start earthworm
   A. Open a command prompt or xterm and go to c:\earthworm\v6.3\run_symres\params
...
6. View the data
   A. Open another command prompt and go to c:\earthworm\v6.3\run_symres\params
...
7. Check the logs to see if there are any problems,
   A. Review the contents of the log files in C:\earthworm\v6.3\run_symres\log
