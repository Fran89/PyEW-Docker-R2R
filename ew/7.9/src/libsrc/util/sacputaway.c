 ...
 *
 *    $Id: sacputaway.c 6803 2016-09-09 06:06:39Z et $
 *
 *    Revision history:
 *     $Log$
...
int SACPABase_next_scnl(char *szSta, char * szChan, char * szNet, char * szLoc)
{
  ...
  for (i = (int)strlen(SAC_szSta); i < K_LEN; ++i)
    sacheadp->kstnm[i] = ' ';

  strcpy(sacheadp->kcmpnm, SAC_szChan);
  for (i = (int)strlen(SAC_szChan); i < K_LEN; ++i)
    sacheadp->kcmpnm[i] = ' ';

  strcpy(sacheadp->knetwk, SAC_szNet);
  for (i = (int)strlen(SAC_szNet); i < K_LEN; ++i)
    sacheadp->knetwk[i] = ' ';
  
  strcpy(sacheadp->khole, SAC_szLoc);
  for (i = (int)strlen(SAC_szLoc); i < K_LEN; ++i)
    sacheadp->khole[i] = ' ';
