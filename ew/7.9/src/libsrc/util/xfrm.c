...
int BaseCompareSCNL(XFRMSCNL* scnl1, XFRMSCNL* scnl2) {
	TracePacket* buf1 = GetInBufForMsgBuf(scnl1->inMsgBuf);
	TracePacket* buf2 = GetInBufForMsgBuf(scnl2->inMsgBuf);

	if (scnl1->inEndtime != scnl2->inEndtime) {
		return 1;
	}
	if (scnl1->samprate != scnl2->samprate) {
		return 2;
	}
	if (GetDataSize(buf1) != GetDataSize(buf2)) {
		return 3;
	}
...
int AddSCNL(XFRMWORLD* pXfrm, SCNL *inSCNL, SCNL *outSCNL, int *jSta) {
   ...
   /* Need to allocate more */
      ...
      if ((pXfrm->scnls = (char *) realloc (pXfrm->scnls,
                         (maxSta * (pXfrm->scnlRecSize)))) == NULL)
      {
    ...
    scnls = pXfrm->scnls;
    nSCNL = pXfrm->nSCNL;
    newSCNL = (XFRMSCNL*)( scnls + nSCNL*(pXfrm->scnlRecSize) );
    *jSta = nSCNL;
 ...
 *****************************************/
  if( msgtype == pXfrm->xfrmEWH->typeTrace2 ) 
  {
     if( (pPkt->trh2.sta  == NULL) || 
         ...
         (pPkt->trh2.loc  == NULL)    )
     {
       logit ("t",  "%s: invalid (NULL) parameters to matchSCNL\n", pXfrm->mod_name);
       return( -2 );
     }
...
thr_ret BaseXfrmThread (void* args) {
  ...
  return (NULL); /* should never reach here unless there is a break in the continuous loop */
                    ...
                    unsigned char msgtype, TracePacket *outBuf)
{
  ...
  if ( pSCNL->inEndtime != 0.0 )
  {
    ...
    {
      logit("t","%s: sample rate changed in <%s.%s.%s.%s> at %lf from %lf to %lf; exiting.\n",
            pDb->mod_name,
    ...
    {
      logit("t","%s: datatype changed in <%s.%s.%s.%s> at %lf from %s to %s; exiting.\n",
            pDb->mod_name,
            pSCNL->inSta, pSCNL->inChan, pSCNL->inNet, pSCNL->inLoc, inBuf->trh2.starttime,
