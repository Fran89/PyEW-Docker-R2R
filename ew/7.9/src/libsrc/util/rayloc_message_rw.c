 ...
 *
 *    $Id: rayloc_message_rw.c 2051 2006-01-17 21:38:06Z friberg $
 *
 *    Revision history:
 *     $Log$
...
void
	rayloc_logRayloc(RAYLOC_MESSAGE_HEADER_STRUCT *p_struct)
	{
		...
		logit( "",
		    "RAYLOC MSG (v. %d) message (type %d ; mod %d instId %d) event id = %d origin = %s\n",
		...
		logit("",    "%6.1f %3d %+2d %6.1f %3d %+2d %6.1f %3d %+2d \n",
		...
		for (i = 0; i < p_struct->numPicks; i++)
		{
			logit("", "%10d %5s %3s %2s %2s %8s %+4.1f %5.1f %3d ",
