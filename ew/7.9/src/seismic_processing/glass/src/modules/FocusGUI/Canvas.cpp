...
void CCanvas::Focus() {
  ...
  //memset(TTTUsedArray,0,sizeof(TTTUsedArray));
	if(!dcShadow)
		return;
	GetClientRect(hWnd, &r);
		...
		for(ittt=nttt-1; ittt>PHASE_Unknown; ittt--) 
    {
      ...
      // for each depth iteration
				...
				if(!pttt)
        {
				...
				//OLD:  focus[nt*iz+it] += Zerg((tmin + it*tmesh - tres)/wzerg);

			//	TRACE("%4d %8s %6d %10.2f %2d %6s %10.2f\n",
			//		i, pck->sSite, ittt, tres, ttt->iTTT, ttt->sPhs, ttt->dT);
